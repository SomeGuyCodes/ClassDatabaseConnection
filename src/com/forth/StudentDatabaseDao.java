                ...
                switch(status) {
                    ...
                    case 1:
                        ...
                        if(count>10){
                            ...
                            String sqlz = "SELECT COUNT(*) FROM studentInfo;";
                            ResultSet rsz = stmt0.executeQuery(sqlz);
                            rsz.next();
                            int countz = rsz.getInt(1);
                            Statement stmt1 = conn.createStatement();
                            String entry = "SELECT AccountId FROM studentInfo;";
                            ResultSet rst = stmt1.executeQuery(entry);
                            rst.next();
                            int entry0 = rst.getInt(sqlz+1);
                            for(int you=0; you>countz+1; you++){
                                if(id == entry0){
                                    id=createId();
                                    break;
                                }
                            ...
                            String sqlx = "SELECT AccountId FROM studentInfo;";
                            ResultSet rsx = stmtx.executeQuery(sqlx);
                                int AccountId = rsx.getInt(sqlx);
                            int countx = rs.getInt(1);
                             String sqlEntry = "INSERT INTO `studentInfo` (`AccountId`,`firstname`,`lastName`,`phoneNumber`" +
                        "" +
                        ",`ssn`,`gpa`,`email`) VALUES (AccountId,firstName,lastName,phoneNumber,ssn,gpa,email);";
